...
const find_power = (power_player, club_info, shot, power_shot, distancia, altura, vento, angulo, terreno, spin, curva, slope, mira = undefined, percent = undefined) => {
    ...
    do {
        ...
        // Tira o limite de 100% e coloca para 130% o máximo
...
function calc(user_input) {
    ...
    if (f[index_f].power != -1) {
        ...
        result = {
            error: false,
            message: `${(f[index_f].power * 100).toFixed(1)}%, ${(f[index_f].power_range * f[index_f].power).toFixed(1)}y, ${((desvioByDegree(f[index_f].desvio, distance) / 0.2167)).toFixed(2)}pb`,
            desvio: desvioByDegree(f[index_f].desvio, distance),
            percent: (f[index_f].power * 100),
            caliper: (f[index_f].power_range * f[index_f].power),
            hwi: desvioByDegree(f[index_f].desvio, distance) / wind / (Math.sin(degree * Math.PI / 180)),
            spin: spin
        }

        // Verifica o caliper
        if (Number(result.caliper.toFixed(1)) > (f[index_f].power_range + 0.1))
            result.caliper = -1; // Não chega
        ...
        result = { 
            error: true,
            message: 'Nao conseguiu calcular a tacada, nao chega ou eh muito fraco.' 
        }
    }
